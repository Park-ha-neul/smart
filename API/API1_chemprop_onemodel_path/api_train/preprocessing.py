...
from config import PreConfig, get_pre_paths_dict  # 플랫폼 내부에선 이렇게 받아들일까?
...
# classification -> regression (float)
def preprocessing_bio_bi_to_float(df):
    def to_float(row, col):
        v = row[col]
        float_v = float(v)
        return float_v

    clf_cols = PreConfig.classification_cols
    for col_ in clf_cols:
        df[col_] = df.apply(lambda row: to_float(row, col_), axis=1)
    return df
...
def to_data_path(df, tm):
    ...
    save_path_dict = get_pre_paths_dict(tm)

    # 각 데이터프레임을 csv로 지정한 경로에 저장한다.
    df.to_csv(save_path_dict['regression'], encoding='utf-8', index=False)

    return save_path_dict


def split_data_and_datapath(df, tm):
    # classification set 포함된 데이터셋 나누기
    # train, test를 나누고 test셋은 classification test할 때 쓰기.
    train_set, test_true_set = train_test_split(df, test_size=0.2, random_state=42)

    save_path_dict = get_pre_paths_dict(tm)

    train_set.to_csv(save_path_dict['regression'], encoding='utf-8', index=False)
    test_true_set.to_csv(save_path_dict['test_classification'], encoding='utf-8', index=False)
...
def preprocess(tm):
    ...
    for i in os.listdir(data_path):
        if '_data_.csv' not in i:
            ...
            df6 = preprocessing_bio_bi_to_float(df5)
            save_path_dict = split_data_and_datapath(df6, tm)
            return save_path_dict

# final.csv : preprocessed csv
