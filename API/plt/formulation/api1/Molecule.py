...
def SmilesToImg(smiles,png_path):
    try:
        m = Chem.MolFromSmiles(smiles)
        path = png_path+now.strftime("%Y%m%d%H%M")+ '.png'
        Chem.Draw.MolToFile(m,path)
        return path
    except:
        return 'Fail'
...
'''
=>smiles to mol, mol to image 방식으로 바꿔서 진행하겠다. 아래 방식은 사용하지 않겠다.
...
def SmilesToSdf(smiles, sdf_path):
    ...
    print('Here is the data_frame from smiles : ;',data_frame)
