...
def init_svc(im):
    ...
    logging.info(f'---IT TOOK {end-start} SECONDS TO LOAD MODEL---') # 0413
    params['model_loading_time'] = str(end-start)+' seconds'
    return params
...
def inference(df, params, batch_id):
    ...
    logging.info(f"Model Loading Time : {params['model_loading_time']}") # 0419
    response = {}
    ...
    if '{}' not in errors_or_input:  # request 에러가 아닐 경우
        ...
        if '{}' in [errors_or_smiles]:
            ...
            # 물성예측
            predicted_regression_dict = predict([[smiles]], params)  #
