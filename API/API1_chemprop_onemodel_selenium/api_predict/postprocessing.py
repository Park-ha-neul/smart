...
def postprocess(smiles, predicted_regression_dict):
    ...
    bio = predicted_regression_dict[InferenceConfig.Classification.bio]

    ac = predicted_regression_dict[InferenceConfig.Pka.most_acidic]
    bs = predicted_regression_dict[InferenceConfig.Pka.most_basic]
    res = {
        'Bioavailability': (lambda x: 1 if x > 0.5 else 0)(bio),
        'pKa': (lambda x: 1.0 if x > 0.5 else 0.0)(ac),
        'pKb': (lambda x: 1.0 if x > 0.5 else 0.0)(bs),
        'Bi Acidic': bi_ac,
        'Bi Basic': bi_bs
    }
    ...
    for i in ['Most Basic', 'Most Acidic']:  # pka, pkb로 naming하는게 맞는건지?
        res.pop(i)
