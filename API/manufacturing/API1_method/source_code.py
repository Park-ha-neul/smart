import sqlite3
import collections
# conn = sqlite3.connect('D:/project/2021/스마트제형설계/제조공정/api1/db/method.db', check_same_thread=False)
    ...
    if len(code) > 0:
        ...
        if formulation not in db_formulation_list:
            ...
            for i in range(len(values)):
                # 조건을 현재는 2개 이상인 method 만 보여주게끔 되어있는데 이를 1개만 만족해도 보여주는걸로 진행하려면 아래 0 -> 1로 변경
                if values[i] > 1:
