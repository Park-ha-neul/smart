...
from api_predict.chemprop_predict import predict
from config import PredictArgsLoadModel
...
import os
# os.setenv('CUDA_VISIBLE_DEVICES', 'none') - 오류나서 제외함. 0518
...
def inference(df, params, batch_id):
    """
     ...
     :param df: df.values[0,0] -> request :  reqid,type,value,path
         ...
         type: input의 type : sdf binary hex data, smiles 셋 중 하나

         value : input의 실제값 : path/to/sdf/file, ['ccc'] 등 셋 중 하나 \
    ...
    # input 값 변화: UI에서 input값 smiles로 변환한 후에 inference()에 넘겨줌. reqid, path 제거함.
    """
        ...
        try:
            if request['type'] in ['smiles', 'sdf']:
                ...
                mol_img_dict = {'molecule': {
                    ...
                    'generated Molecule': "/home/data/t3q/uploads/pharmAi/molecule_drawing.png" # ui test용으로 하드코딩해서 넣음0518
                }
                }
                ...
                log.info(f'RESPONSE TIME : {end - start} SEC')
