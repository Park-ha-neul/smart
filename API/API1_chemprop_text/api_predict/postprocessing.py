...
def power_and_round(res):
    ...
    for k, v in res.items():
        ...
        if type(v) != str:
            # res1[k] = np.round(v, 2)
            res1[k] = round(v, 2) # numpy 로 하지 않음 0518
        else:
            res1[k] = v
...
def postprocess(smiles, predicted_regression_dict):
    ...
    res_clf = {
        bio_key: (lambda x: int(1) if x > 0.5 else int(0))(bio),
...
if __name__ == "__main__":
    ...
    out = postprocess('CCC(=O)OC1(C(C)CC2C1(C)CC(O)C1(C2CCC2=CC(=O)C=CC12C)F)C(=O)CCl', res)
    print(out)

    prop =out['properties']
    print(prop)
    # for num, j in enumerate(prop):
    #     print(f'------{num, j}--------')
    #     prop_ = prop[j]
        # if isinstance(prop_,np.int64):
        #     prop_altered = int(prop_)
        #
        #     json_data = dumps(prop_altered, indent=2)
        #     with open(f'./jsons/output_{j}.json', 'w') as f:
        #         f.write(json_data)
        #     print('json done \n')

    # out.pop('properties')
    for num, j in enumerate(out):
        print(f'------{num, j}--------')
        json_data = dumps(out[j], indent=2)
        with open(f'./jsons/output_{j}.json', 'w') as f:
            f.write(json_data)
        print(out[j])